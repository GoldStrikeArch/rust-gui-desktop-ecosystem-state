...
|---|---|---|---|
| async_integration | **assembled** | observed | tokio =1.52.3 (1 worker) + `upgrade_in_event_loop`. The probe (above) proves `spawn_local` is not viable for tokio-backed I/O — Slint gives you a futures executor but no reactor — which the `spawn_local` rustdoc does document, prescribing `async_compat::Compat` for tokio futures (correction 2026-08-30: an earlier draft claimed nothing warns you). |
