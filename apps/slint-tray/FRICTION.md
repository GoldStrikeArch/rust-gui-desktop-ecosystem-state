...
| global_hotkey | **assembled** | `global-hotkey` crate (Carbon `RegisterEventHotKey`). Its event channel has no waker integration with Slint's loop → poll `GlobalHotKeyEvent::receiver().try_recv()` in a 50 ms `slint::Timer`. Verified: Cmd+Shift+9 toggles visibility while unfocused/hidden. Contention with sibling test apps registering the same combo was observed but not characterised (two other apps in this round saw every registrant fire); an earlier draft's "only one process can own the combo" was an inference, not a measurement. |
