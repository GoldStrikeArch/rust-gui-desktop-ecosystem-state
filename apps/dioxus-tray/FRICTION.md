...
|---|---|---|
| tray | **built-in** | `dioxus::desktop::trayicon::init_tray_icon(menu, icon)` (re-exported tray-icon crate) called once in a `use_hook`; `None` icon gives the Dioxus logo. Left-click shows/focuses the window (framework behaviour, configurable via `Config::with_tray_icon_show_window_on_click`), right-click opens the menu. Verified live: icon visible in menu bar, left-click restored the hidden window, menu items fire through the callback shared by both menu hooks (per the trap under native_menubar they actually arrive on the muda hook), tray "Quit" (muda `PredefinedMenuItem::quit`) really exits. |
...
| native_menubar | **built-in** | muda `Menu` passed via `Config::with_menu`; first submenu becomes the macOS application menu; predefined items give ⌘Q quit, ⌘W close, full Edit clipboard roles; custom items take `Accelerator` (`"CmdOrCtrl+N".parse()`). Verified via AX: menus "Apple, dioxus-tray, File, Edit" exist; clicking File→New fired our id. **Trap:** tray-icon re-exports the *same* muda, and muda's `MenuEvent::set_event_handler` slot is a process-global `OnceCell` (first registration wins; later calls are silently ignored). dioxus-desktop 0.7.9 registers its menubar receiver (`app.rs:449`) before its tray-menu receiver (`app.rs:470`), so **tray-menu clicks arrive as `MudaMenuEvent` and `use_tray_menu_event_handler` never fires** — tracked upstream as DioxusLabs/dioxus#4495. This app registers BOTH hooks with one shared callback, which is why it works either way. (Correction 2026-08-30: the first version of this note had the direction inverted — it claimed menubar events arrived as tray events; a source re-check of muda 0.17.2/0.19.3 and dioxus-desktop 0.7.9 established the above.) Unbundled binaries show the process name ("dioxus-tray"), not a display name, as the app menu title. |
