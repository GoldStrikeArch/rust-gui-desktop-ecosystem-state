...
| cjk_render | **NOT-ACHIEVABLE out of the box** (headline) | [ZH] and [JA] Han/kana render as PURE TOFU — fontique 0.7.0's fallback fails to resolve PingFang on macOS at this rev (CoreText names it, but it has no readable font file and 0.7 rejects families missing from its directory scan; fixed upstream in fontique 0.8.0 — checked 2026-08-30) — while [KO] Hangul, [HI] Devanagari (with conjuncts) and [TH] Thai all render fine. 世界 inside the [MIXED] line is tofu too, in both the label pane and the editor. The same stack renders CJK on other platforms, so this is a macOS fallback-resolution bug rather than a missing feature; no app-side fix short of bundling a CJK font (not done — the gap is the datum). Corroborating symptom: ⌘/⇧ glyphs also tofu (floem-tray). |
